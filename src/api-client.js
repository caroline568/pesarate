...
async function request(
  ...
  try {
    ...
    console.error(
      `Can't reach the PesaRate server at ${BASE}: ${error.message}`
    );

    throw new Error(
      `Can't reach the PesaRate server at ${BASE}. Check that the backend is running and VITE_API_URL is set correctly.`,
      { cause: error }
    );
  }
...
export const conversionsApi = {
  ...
  compare: ({ from, to, amount, channel }) =>
    request(
      `/conversions/compare?from_currency=${encodeURIComponent(
        from
