...
SUPPORTED_CHANNELS = {"Remitly", "Wise", "Bank", "Cash pickup"}
...
def _build_comparison(source_currency, target_currency, amount, channel):
    """Build the actual provider-vs-mid-market calculation used by PesaRate."""
    ...
    if channel == "Cash pickup":
        # Wise documents that Comparison currently estimates bank-transfer
        # pay-in/pay-out. Never invent a cash-pickup markup.
        raise ValueError(
            "Cash pickup comparisons are not currently supported by the Wise Comparison API."
        )

    mid_market_rate = _get_mid_market_rate(source_currency, target_currency)
    mid_market_result = amount * mid_market_rate
    wise_data = _request_wise_comparison(
        source_currency, target_currency, amount, mid_market_rate
    )
    provider = _select_provider_quote(wise_data, channel)

    provider_result = provider["received_amount"]
    provider_rate = provider["rate"]
    difference = mid_market_result - provider_result
    cost_percent = (difference / mid_market_result * 100) if mid_market_result else 0.0

    markup_percent = provider["markup"]
    if markup_percent is None and provider_rate:
        markup_percent = ((mid_market_rate - provider_rate) / mid_market_rate) * 100
    ...
    return {
        ...
        "dateCollected": provider["date_collected"],
        "deliveryEstimation": provider["delivery_estimation"],
        "sourceCountry": provider["source_country"],
        "targetCountry": provider["target_country"],
        "isMidMarket": provider["is_mid_market"],
        "comparisonDisclaimer": (
            "Provider comparison is an estimate based on collected pricing. "
            "Wise currently models bank-transfer pay-in and pay-out."
        ),
    }
